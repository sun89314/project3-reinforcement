...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(legalActions) == 0:
            ...
            return ActionCounter[ActionCounter.argMax()]
    ...
    def update(self, state, action, nextState, reward):
        ...
        prevQ = (1 - self.alpha) * self.getQValue(state, action)
        if nextState:
            self.Qvalues[(state, action)] = prevQ + reward + \
                self.discount * self.alpha * self.getValue(nextState)
        else:
            self.Qvalues[(state, action)] = prevQ + reward
